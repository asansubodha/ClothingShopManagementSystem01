            ...
            else
            {
                if(connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        string selectProdID = "SELECT * FROM products WHERE ProductID = @ProductID";

                        using (SqlCommand selectPID = new SqlCommand(selectProdID, connect))
                        {
                            selectPID.Parameters.AddWithValue("@ProductID", adminAddProducts_id.Text.Trim());
                            ...
                            {
                                string insertData = "INSERT INTO products (ProductID, ProductName, Type, " +
                                    "Stock, Price, Status, Image, DateInsert) VALUES(@prodID, @prodName" +
                                    ", @prodType, @prodStock, @prodPrice, @prodStatus, @prodImage, @dateInsert)";

                                DateTime today = DateTime.Today;

                                string path = Path.Combine(@"C:\Users\hp\repos\Product_Directory\"
                ...
                if(check == DialogResult.Yes)
                {
                    if(connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE products SET ProductName = @prodName" +
                                ", Type = @prodType, Stock = @prodStock, Price = @prodPrice, Status = @prodStatus" +
                                ", DateUpdate = @dateUpdate WHERE ProductID = @prodID";
                            DateTime today = DateTime.Today;
                ...
                if (check == DialogResult.Yes)
                {
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE products SET date_delete = @dateDelete WHERE ProductID = @prodID";
                            DateTime today = DateTime.Today;
